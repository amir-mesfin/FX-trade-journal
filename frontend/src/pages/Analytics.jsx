  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Sessions, pairs, and strategy tags</p>
      </div>
      ...
      {stats && (
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
              <p className="text-xs uppercase text-slate-500">Avg R:R</p>
              <p className="mt-2 text-lg font-semibold text-white">{stats.avgRR ?? '—'}</p>
            </div>
          </div>

          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
            <h2 className="text-sm font-medium text-slate-300">P/L by session (UTC buckets)</h2>
            {sessionData.length === 0 ? (
              <p className="mt-6 text-center text-sm text-slate-500">No closed P/L data yet.</p>
            ) : (
